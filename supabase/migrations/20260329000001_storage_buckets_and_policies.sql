...
-- Authenticated users: read/write/delete only under their own user-id prefix.
...
CREATE POLICY movemark_storage_inspection_media ON storage.objects
  ...
  USING (bucket_id = 'inspection-media' AND split_part(name, '/', 1) = auth.uid()::text)
  WITH CHECK (bucket_id = 'inspection-media' AND split_part(name, '/', 1) = auth.uid()::text);
...
CREATE POLICY movemark_storage_maintenance_media ON storage.objects
  FOR ALL TO authenticated
  USING (bucket_id = 'maintenance-media' AND split_part(name, '/', 1) = auth.uid()::text)
  WITH CHECK (bucket_id = 'maintenance-media' AND split_part(name, '/', 1) = auth.uid()::text);
...
CREATE POLICY movemark_storage_exports ON storage.objects
  FOR ALL TO authenticated
  USING (bucket_id = 'exports' AND split_part(name, '/', 1) = auth.uid()::text)
  WITH CHECK (bucket_id = 'exports' AND split_part(name, '/', 1) = auth.uid()::text);

DROP POLICY IF EXISTS movemark_storage_leases ON storage.objects;
CREATE POLICY movemark_storage_leases ON storage.objects
  FOR ALL TO authenticated
  USING (bucket_id = 'leases' AND split_part(name, '/', 1) = auth.uid()::text)
  WITH CHECK (bucket_id = 'leases' AND split_part(name, '/', 1) = auth.uid()::text);
...
CREATE POLICY movemark_storage_deposit_receipts ON storage.objects
  FOR ALL TO authenticated
  USING (bucket_id = 'deposit-receipts' AND split_part(name, '/', 1) = auth.uid()::text)
  WITH CHECK (bucket_id = 'deposit-receipts' AND split_part(name, '/', 1) = auth.uid()::text);
...
CREATE POLICY movemark_storage_listing_screenshots ON storage.objects
  FOR ALL TO authenticated
  USING (bucket_id = 'listing-screenshots' AND split_part(name, '/', 1) = auth.uid()::text)
  WITH CHECK (bucket_id = 'listing-screenshots' AND split_part(name, '/', 1) = auth.uid()::text);
